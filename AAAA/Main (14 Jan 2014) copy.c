...
unsigned char AttentionFlag = 0;
unsigned int AttentionCnt = 0;
...
ram near unsigned int PreviousFaderCvSet;
volatile unsigned int FADER_IN;
volatile unsigned char NEW_FaderCvSet = 0;
...
void InterruptHandlerHigh () 
{
	...
	{			
		...
		{
			FaderPosition = Read_Fader();                                       // Sample Fader CV
...
{	
	...
	if (I2Cflag)                                            // Move to ISR
		...
		if (1 == 1 && snapshot_flag == 0)					// control byte for read/write ???
			{
			FaderCvSet = indata.data[1] & 3;                 // Fader CV to chase
			FaderCvSet = (FaderCvSet<<8)+indata.data[0];          
			PreviousFaderCvSet = Goal;                               // Last Goal?
			NEW_FaderCvSet=0;                                // ?
			if(FaderCvSet!=FADER_IN)                         // ?
				{
				NEW_FaderCvSet=1;                            // Test for new CV in
				}
			FADER_IN = FaderCvSet;                           
			FADER_DIFF = FaderCvSet - FaderPosition;             // Get difference from Chase CV and ??
			}
