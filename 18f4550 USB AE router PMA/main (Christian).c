...
void Backplane_Comm(unsigned char LockedQframe) 
{
    ...
    for(k=0;k<3;k++)													// Each USB packet has 48 bytes for 24 channels (3 banks).
    {
        ...
        for(j=0;j<5;j++)												// waitloop to ensure the bank is responding
        {
            ;
        }
    ...
    for(i=0;i<8;i++)      // This can be deleted when the app supports the new motor fader                                   
	{
		usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)]=0;
		usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)+1]=0;
	}
...
{   
    ...
    if((USBDeviceState < CONFIGURED_STATE)||(USBSuspendControl==1)) {USB_LED=0;return;}
	USB_LED=1;
	...
	if(load_USB_flag)                                                               // flag for new data to send to host
	{
	    ...
	    {
	DEBUG1=1;
	if(LockedQframe==0){DEBUG3=1;MIDI_LED=!MIDI_LED;}
	else DEBUG3=0;
				...
				for(i=0;i<64;i++)
					{
						TempIn[i]=usb_out.SLOT[LockedQframe].BYTES[i];              				 // 
					} 
