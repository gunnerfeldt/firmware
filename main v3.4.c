...
#define DEBUG3			LATEbits.LATE2 // Pin 10
...
// routing 1 of 2 - 22 switches
const rom unsigned char address0[] = {	0x77,0x37,0x57,0x17,0x67,0x27,0x47,0x07,
										0x73,0x33,0x3D,0x7D,0x0B,0x4B,0x2B,0x6B,
										0x43,0x03,0x75,0x35,0x55,0x15,0x00,0x00};
// routing 2 of 2 - 19 switches, 3 pots
const rom unsigned char address1[] = {	0x19,0x59,0x39,0x79,0x0D,0x4D,0x2D,0x6D,
										0x1D,0x5D,0x23,0x63,0x13,0x53,0x31,0x05,
										0x51,0x21,0x71,0x11,0x61,0x2E,0x00,0x00};

// dynamics 1 of 2 - 7 switches
const rom unsigned char address2[] = {	0x5E,0x09,0x29,0x65,0x3A,0x0E,0x01,0x00,
                  						0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// dynamics 2 of 2 - 6 pots
const rom unsigned char address3[] = {	0x6E,0x1E,0x3E,0x7E,0x49,0x69,0x00,0x00,
                  						0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// eq 1 of 2 - 8 switches, 6 pots
const rom unsigned char address4[] = {	0x41,0x4E,0x7A,0x4A,0x0A,0x7C,0x72,0x32,
                  						0x76,0x36,0x56,0x16,0x66,0x26,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// eq 2 of 2 - 7 pots (last one is type)
const rom unsigned char address5[] = {	0x06,0x5A,0x1A,0x6A,0x2A,0x4A,0x25,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// sends 1 of 2 - 15 switches
const rom unsigned char address6[] = {	0x12,0x62,0x42,0x02,0x38,0x34,0x78,0x2C,
                  						0x4C,0x6C,0x3C,0x5C,0x50,0x10,0x30,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// sends 2 of 2 - 6 pots
const rom unsigned char address7[] = {	0x52,0x22,0x14,0x0C,0x1C,0x70,0x7E,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// faders 1 of 2 - 11 switches
const rom unsigned char address8[] = {	0x60,0x40,0x48,0x58,0x28,0x08,0x24,0x20,
                  						0x04,0x54,0x74,0x00,0x00,0x00,0x00,0x00,
                  						0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// faders 2 of 2 - 8 pots
const rom unsigned char address9[] = {	0x68,0x18,0x44,0x64,0x3B,0x45,0x1B,0x5B,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
										0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
// easy access to the tables
const rom unsigned char *addr[10];
...
unsigned char grpSnapshotState = 0;
unsigned char grpRecallState = 0;
unsigned char dataState = 0;
unsigned char totalRecall = 0;
unsigned char reg = 0;
unsigned char bank = 0;
unsigned char vcaGrpLevel[16];
...
void setMPX (unsigned char);
void dropOutOfTR (void);
void EXI2Ccmd(unsigned char);
void EX08cmd(unsigned char);
void EX08grpRecall(void);
...
/** VECTOR REMAPPING ***********************************************/
	...
	unsigned int setupTimer=0;
	unsigned char USB_plugged =0;
	...
	void YourHighPriorityISRCode()
	{
	 ...
	 if(PIR1bits.TMR1IF)
		{
		if(USBDeviceState == ATTACHED_STATE && !USB_plugged)
		{
			USB_plugged=1;
		}
		if(USBDeviceState == DETACHED_STATE && USB_plugged)
		{
			USB_plugged=0;
		}
//		MIDI_LED=1;
...
void Backplane_Comm(unsigned char LockedQframe)
{
  ...
  unsigned char TRscanCntr = 0;
	...
	usb_in.SLOT[j].BYTES[0] = 0x00;				// Setup Data CMD_GROUP  ???? What ?? j ??


// Fixed length is good. It will make every Q frame the same.
// 41 is actually the max amount of addresses per region
// addrLength = region[reg].switches+region[reg].pots;

addrLength = 22;
//reg=1;bank=0;

/******************************************************************/
//  Total Recall related back plane communications
//  Slot 13 (serial port) - set address
//  Slot (bank) - read data
//  Pack USB buffer ready for next Q frame
/******************************************************************/
	if(totalRecall==2)
	{
		// regMap is index for the TR96 address maps
		// Every region flips between 2 maps. The flip is
		// taken from the Q frame
		regMap = (reg<<1)+(LockedQframe&1);
	  USBDeviceTasks();
       COMM_READ;
       MUX_ENABLE=1;
    // loop through all MUX addresses
	for(l=0;l<(addrLength+1);l++)
	{
		if(l<(addrLength+1))
		{
	      // Start with TR96 setting an address
	  			Address_MUX(12);
	  			MUX_ENABLE=0;
	        // The serial port always Write/Read 4 bytes
	        // so a 4 byte dummy read is necessary
	        for(i=0;i<4;i++)
	        {
	            usb_in.SLOT[0].BYTES[56+i]=GET_PORT;
	            COMM_CLOCK = 1;
	            WAIT4;
	            COMM_CLOCK = 0;
	        }
	        // the addresses are stored as a table in rom
	        // 0xF1 is known as MUX addresses on the serial port.
	        // Pad with two zeros
	        COMM_WRITE;
	        for(i=0;i<4;i++)
	        {
	            if(i==0)SET_PORT=*(addr[regMap]+l);
	            if(i==1)SET_PORT=0xF1;
	            if(i==2)SET_PORT=0;
	            if(i==3)SET_PORT=0;
		...
		if(!l)
		{
			for(waitClock=0;waitClock<85;waitClock++)
				{;}
		}
		else
		{
	        // An address is set. Let's scan the object
	        Address_MUX(bank);
	        MUX_ENABLE=0;
	        COMM_READ;
	        // This is a trim to expand the interval between addresses
	        // the SSL & I2C need some extra time                           ***** this need to be tuned right !!!! *****
	//        for(j=0;j<1;j++){;}
			WAIT8;
			WAIT8;

			// region[] stores no of switches
			...
			if(TRscanCntr<region[regMap].switches)
			{
				TempIn[TRscanCntr]=0;
				for(i=0;i<8;i++)
				{
					WAIT4;
					// Pack switches as bits
					if(GET_PORT>32)TempIn[TRscanCntr]+=(1<<i);
//					TempIn[TRscanCntr]=255;
					COMM_CLOCK = 1;
					WAIT8;
					COMM_CLOCK = 0;
					WAIT8;
			...
			{
				if(TRscanCntr<(region[regMap].switches+(region[regMap].pots*8)))
			//	if(TRscanCntr<61)
				{
					for(i=0;i<8;i++)
					{
						TempIn[TRscanCntr]=GET_PORT;
						COMM_CLOCK = 1;
						WAIT8;
						COMM_CLOCK = 0;
						WAIT8;
			...
			// EX08 card must know when to jump out of Total Recall mode
			// therefor we send one byte with a command after the 8 reads
			// 0x00 = stay in TR mode, 0xFF = jump out
			COMM_WRITE;
	    		// 0 will tell EX08 to stay in TR mode
			WAIT2;
			COMM_CLOCK = 1;
			WAIT2;
			COMM_CLOCK = 0;
    ...
    // Packet must contain CMD, region and bank when sent to host
		usb_in.SLOT[LockedQframe].BYTES[0]=0x08;
		usb_in.SLOT[LockedQframe].BYTES[2]=regMap+(bank<<4);
    // copy buffer
		for(k=0;k<60;k++)
		{
			usb_in.SLOT[LockedQframe].BYTES[3+k]=TempIn[k];
		}
    // Upgrade state to 3
		totalRecall=3;
	}
	...
	// SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA SETUP DATA
	//
	...
	{
		o=0;
		    for(k=0;k<13;k++)													// Each USB packet has 48 bytes for 24 channels (3 banks).
		    {
	          USBDeviceTasks();												// Check USB pipe
	          Address_MUX(k);													// Qframe (0-3) * 3 + k(0-2)
	          MUX_ENABLE=0;													// Strobe the MUX address
	          COMM_READ;														// Set parallel port to READ
	          WAIT8;												// waitloop to ensure the bank is responding
	          for(i=0;i<4;i++)												// 2 bytes x 8 channels from each card
	          {																// Read PORT
	            TempIn[o]=GET_PORT;											// Store parallel data in TempIn
	            COMM_CLOCK = 1;												// Clock
	            WAIT2;											// Wait loop
	            COMM_CLOCK = 0;
	            WAIT4;
		...
		for(j=0;j<1;j++)
		{
			usb_in.SLOT[LockedQframe].BYTES[0] = 0x04;				// Setup Data CMD_GROUP
			usb_in.SLOT[LockedQframe].BYTES[8] = CARD_ID;			// This Cards ID
			usb_in.SLOT[LockedQframe].BYTES[9] = CARD_MAJOR;			// Major
			usb_in.SLOT[LockedQframe].BYTES[10] = CARD_MINOR;			// Minor

		    for(k=0;k<39;k++)
		    {
				usb_in.SLOT[LockedQframe].BYTES[k+11]=TempIn[k];			// copy data
			}
		}
		dataState=4;
	}

// Group Snaphot Group Snaphot Group Snaphot Group Snaphot Group Snaphot Group Snaphot Group Snaphot

	if(grpSnapshotState==2 && LockedQframe==3)
	{
DEBUG2=1;
         Address_MUX(11);													// MF bank
         MUX_ENABLE=0;													// Strobe the MUX address
         COMM_READ;														// Set parallel port to READ
         WAIT8;												// waitloop to ensure the bank is responding
         for(i=0;i<16;i++)												// 2 bytes x 8 channels
	         {																// Read PORT
	            TempIn[i]=GET_PORT;											// Store parallel data in TempIn
	            COMM_CLOCK = 1;												// Clock
	            WAIT2;											// Wait loop
	            COMM_CLOCK = 0;
	            WAIT4;
		...
		MUX_ENABLE=1;
         ...
         WAIT8;												// waitloop to ensure the bank is responding
		usb_in.SLOT[LockedQframe].BYTES[0] = 0x28;				// Snapshot CMD
         for(i=0;i<16;i++)												// 2 bytes x 8 channels
	         {																// Read PORT
	            	usb_in.SLOT[LockedQframe].BYTES[8+i] = TempIn[i];			// group levels. index = 8 !!!!!
	    		}
...
DEBUG2=0;
	...
	if(grpSnapshotState==1 && LockedQframe==3)
	{
		grpSnapshotState=2;
	}
...
//	if(usb_out.SLOT[LockedQframe].CMD.CMD_GROUP==0 && !dataState)
	if(automationState)
	{
DEBUG3=1;
...
// routing 1 of 2 - 22 switches
  region[0].switches 	= 22;
  region[0].pots 	= 0;
// routing 2 of 2 - 19 switches, 3 pots
  region[1].switches 	= 19;
  region[1].pots 	= 3;
// dynamics 1 of 2 - 7 switches
  region[2].switches 	= 7;
  region[2].pots 	= 0;
...
// eq 1 of 2 - 8 switches, 6 pots
  region[4].switches 	= 8;
  region[4].pots 	= 6;
// eq 2 of 2 - 5 pots (last one is type)
  region[5].switches 	= 0;
  region[5].pots 	= 7;
// sends 1 of 2 - 15 switches
  region[6].switches 	= 15;
  region[6].pots 	= 0;
// sends 2 of 2 - 6 pots
  region[7].switches 	= 0;
  region[7].pots 	= 6;
// faders 1 of 2 - 11 switches
  region[8].switches 	= 11;
  region[8].pots 	= 0;
// faders 2 of 2 - 8 pots
  region[9].switches 	= 0;
  region[9].pots 	= 8;
...
//DEBUG2=1;
	    ...
	    {
	//		usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x00;
			...
			if(automationState)
			{
		        ...
		        usb_in.SLOT[LockedQframe].MTC_PROP.FPS = FPScode;			// reset data Request
			}
			if(automationState)
			{
			...
			if(dataState==4)
			{
			//	usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x04;
				dataState=0;			// reset data Request
				automationState=1;
			}
			if(grpSnapshotState==3)
			{
				grpSnapshotState=0;			// reset state
				automationState=1;
			}
			if(totalRecall==3)
			{
				usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x08;
				totalRecall=2;			// reset data Request
			}
...
*/
        			USBInHandle = HIDTxPacket(HID_EP,(BYTE*)&usb_in.SLOT[LockedQframe],64);  // Point to USB>Host buffer
...
//		if((TEMP_BUF.BYTES[0])==1)dataState=1;
//		else dataState=0;

		if(!appRunning)
		{
		//	setupTimer = 200;				// uncomment this to have a built in harware report
		}
		if(setupTimer>0)
		{
			setupTimer--;
		...
		// Groups Snapshot Command
		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x28 && automationState)
		{
			for(i=0;i<8;i++)
			{
				usb_out.SLOT[3].BYTES[41+(i*2)]=0x13;
			}
			grpSnapshotState = 1;
		}
/*
		// Groups Recall Command
		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x48 && automationState)
		{

			for(i=0;i<8;i++)
			{
				usb_out.SLOT[3].BYTES[40+(i*2)]=usb_out.SLOT[slot].BYTES[8+(i*2)];
				usb_out.SLOT[3].BYTES[41+(i*2)]=((usb_out.SLOT[slot].BYTES[9+(i*2)])<<5)+0x14;
			}
      		grpRecallState = 1;
...
*/
		...
		// Total Recall Command
 		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x08)
		{
			// This probably means new bank or new region.
			// must drop out EX08 card if bank changes
			if(totalRecall)
			{
			// This will take EX08 out of TR mode
				dropOutOfTR();
				totalRecall=1;
				automationState = 0;
				reg = usb_out.SLOT[slot].BYTES[2]&0x0F;
				bank = (usb_out.SLOT[slot].BYTES[2]&0xF0)>>4;
				EX08cmd(0x12);
				setMPX(1);		// Turn on mpx
				totalRecall=2;
			}
		// Total Recall Command start
			else
			{
				totalRecall=1;
				automationState = 0;
		...
		// Total Recall Command end
 		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x18 && totalRecall)
		{
			automationState = 1;
			totalRecall=0;
			// Turn of TR96 MPX enable
			setMPX(0);
			// This will take EX08 out of TR mode
			dropOutOfTR();
		}
...
void setMPX (unsigned char mpx)
{
	...
	{
		//      Read PORT
		usb_in.SLOT[0].BYTES[56+i]=GET_PORT;       // Dummy read
		COMM_CLOCK = 1;
		for(j=0;j<4;j++){;}
		COMM_CLOCK = 0;
	}
		...
		for(i=0;i<4;i++)                                               // Write 2 x bytes to each card
		{
		if(i==0)SET_PORT=0x7F+(0x80*mpx);                 // Turns on MPX
		if(i==1)SET_PORT=0xF1;
		if(i==2)SET_PORT=0;
		if(i==3)SET_PORT=0;
...
void EXI2Ccmd(unsigned char cmd)
{
	unsigned char i,j,k,l;

	Address_MUX(12);										// Only serial pic
    ...
    for(i=0;i<4;i++)                                               // Write 2 x bytes to each card
    {
       SET_PORT=cmd;      		// Command for setup
       WAIT4;
       COMM_CLOCK = 1;                                             // Clock
       WAIT4;
       COMM_CLOCK = 0;
