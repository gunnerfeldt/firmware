...
unsigned char TRcardSlot = 0;
unsigned char TRaddress = 0;
unsigned char qframeTimer = 0x98;
...
void AUTOcomm(unsigned char);
void TRcomm(unsigned char);
void sendMPXAddress(unsigned char);
...
//		MIDI_LED=1;
		...
		PIR1bits.TMR1IF = 0;
		TMR1H = qframeTimer;		//0x40 when scaler=1_2. 0x98 when 1_4
		TMR1L = 0x00;		//0x00
//		MIDI_LED=0;
		}
...
//DEBUG2=1;
	...
	//		usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x00;
			...
			if(automationState)
			{
				...
				usb_in.SLOT[(LockedQframe+3)&3].MTC_PROP.STOP=0;
				usb_in.SLOT[(LockedQframe+3)&3].MTC_PROP.RUN=0;
				usb_in.SLOT[(LockedQframe+3)&3].MTC_PROP.JUMP=0;
			}
...
//		if(slot==0)
		{
	 		if(usb_out.SLOT[slot].BYTES[7] == 0xF0)
			{
				autoMode=1;
				setMPX(0);
				qframeTimer = 0x98;
			...
			// Total Recall Command
	 		if(usb_out.SLOT[slot].BYTES[7] == 0xF1)
			{
				setMPX(1);
				autoMode=2;
				qframeTimer = 0x50;
			}
			// Total Recall address set
	 		if(usb_out.SLOT[slot].BYTES[7] == 0xF2)
			{
				TRcardSlot=usb_out.SLOT[slot].BYTES[56];
				TRaddress=usb_out.SLOT[slot].BYTES[57];
//				sendMPXAddress(TRaddress);
			}
		}
...
void AUTOcomm(unsigned char LockedQframe)
{
...
//
		...
		usb_in.SLOT[LockedQframe].BYTES[7]=0xF0;
		usb_in.SLOT[LockedQframe].BYTES[0] = 0x00;				// Automation Data CMD_GROUP
	}
}


void TRcomm(unsigned char qFrame)
{
  	unsigned char i,j,k,o;
	unsigned char TempIn[64];
	unsigned char addCmd = 0;
	unsigned char addrLength = 0;
  	unsigned char TRscanCntr = 0;
	...
	static unsigned char lastCardSlot;
	static unsigned char lastAddress;
	static unsigned char lastAddressBuf;
	static unsigned char lastOutAddress;
	...
	if(1)
	{
		lastOutAddress=TRaddress;

	    USBDeviceTasks();												// Check USB pipe

//		while(l<(addressesEachQframe+1))
		{
			// address
			sendMPXAddress(lastOutAddress);
		
			COMM_READ;														// Set parallel port to READ
		
			//MUX the right bank
			Address_MUX(TRcardSlot);								
			MUX_ENABLE=0;													// Strobe the MUX address		
			WAIT12;
		...
		{
			{
				usb_in.SLOT[qFrame].SSL_DATA[i]=TempIn[i];
			}
		}
		usb_in.SLOT[qFrame].BYTES[7]=0xF2;
		usb_in.SLOT[qFrame].BYTES[56]=lastCardSlot;
		usb_in.SLOT[qFrame].BYTES[57]=lastAddress;
//		usb_in.SLOT[addrFlip].BYTES[0] = 0x01;				// Recall Data CMD_GROUP

		lastCardSlot = TRcardSlot;
		lastAddress = lastAddressBuf;
		lastAddressBuf = lastOutAddress;
	}
}
...
void sendMPXAddress(unsigned char address)
{
