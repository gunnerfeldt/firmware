...
void TRcomm(unsigned char,unsigned char, unsigned char);
void sendMPXAddress(unsigned char,unsigned char);
...
void main(void)
{
	...
	if(BACKPLANE_FLAG && !HoldCardBus)
	{
		...
		if(autoMode==2)
		{
			// card slot, address index, addr flip
			TRcomm(0,addressIndex, LockedQframe);
		}
...
//		if(slot==0)
		{
			...
			{
				addressIndex=usb_out.SLOT[slot].BYTES[8];
			}
		}
...
void TRcomm(unsigned char cardSlot, unsigned char addrIndex, unsigned char addrFlip)
{
  	unsigned char i,j,k,l,o;
	unsigned char TempIn[64];
	unsigned char addCmd = 0;
	...
	unsigned long waitClock=0;
	unsigned char regMap = 0;
	unsigned char addressesEachQframe = 3;
	...
	if(1)
	{
		...
		while(l<(addressesEachQframe+1))
		{
			TMR2=0;
			trStrobe=0;
			// address array, index of arrays
			if(l<addressesEachQframe)sendMPXAddress(addrIndex, l+(addrFlip*6));
			else{WAIT12;WAIT12;WAIT12;WAIT12;WAIT12;}
		
			COMM_READ;														// Set parallel port to READ
		
			//MUX the right bank
			Address_MUX(cardSlot);									
			MUX_ENABLE=0;													// Strobe the MUX address		
			WAIT12;
	
	        for(i=0;i<16;i++)												// 2 bytes x 8 channels from each card
	        {
				TempIn[(l*16)+i]=GET_PORT;									// Store parallel data in TempIn
		...
		{
			{
					usb_in.SLOT[addrFlip].SSL_DATA[i]=TempIn[i];
			}
		}
...
void sendMPXAddress(unsigned char address, unsigned char index)
{
	unsigned char dummy;
	unsigned char regMap = 0;
  	unsigned char i,j;
	...
	for(i=0;i<4;i++)
	{
	    if(i==0)SET_PORT=*(addr[regMap]+index);
	    if(i==1)SET_PORT=0xF1;
	    if(i==2)SET_PORT=0;
	    if(i==3)SET_PORT=0;
