...
DEBUG2=0;
			for(i=0;i<8;i++)
			{
				vcaGrpLevel[(i*2)]=usb_out.SLOT[slot].BYTES[8+(i*2)];
				vcaGrpLevel[(i*2)+1]=((usb_out.SLOT[slot].BYTES[9+(i*2)])<<5)+0x14;
			}
			EX08grpRecall();
			bank=bankSwap;
		}
...
void EX08grpRecall(void)
{
	...
	Address_MUX(11);								// Qframe (0-3) * 3 + k(0-2)
