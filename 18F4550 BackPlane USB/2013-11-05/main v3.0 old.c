...
// routing 1 of 2 - 41 switches
const rom unsigned char address1[] = {	0x77,0x37,0x57,0x17,0x67,0x27,0x47,0x07,
									0x73,0x33,0x3D,0x7D,0x0B,0x4B,0x2B,0x6B,
									0x43,0x03,0x75,0x35,0x55,0x15,0x19,0x59,
									0x39,0x79,0x0D,0x4D,0x2D,0x6D,0x1D,0x1D,
									0x5D,0x23,0x63,0x13,0x53,0x05,0x51,0x21,
                  						0x71};
// routing 2 of 2 - 3 pots
const rom unsigned char address2[] = {	0x11,0x61,0x2E};

// dynamics - 7 switches, 6 pots
const rom unsigned char address3[] = {	0x5E,0x09,0x29,0x65,0x3A,0x0E,0x01,
                  						0x6E,0x1E,0x3E,0x7E,0x49,0x69};
// eq 1 of 2 - 8 switches, 8 pots
const rom unsigned char address4[] = {	0x41,0x4E,0x7A,0x4A,0x0A,0x7C,0x72,0x32,
                  						0x76,0x36,0x56,0x16,0x66,0x26,0x06,0x5A};
// eq 2 of 2 - 5 pots (last one is type)
const rom unsigned char address5[] = {	0x1A,0x6A,0x2A,0x4A,0x25};

// sends 1 of 2 - 15 switches
const rom unsigned char address6[] = {	0x12,0x62,0x42,0x02,0x38,0x34,0x78,0x2C,
                  						0x4C,0x6C,0x3C,0x5C,0x50,0x10,0x30};
// sends 2 of 2 - 6 pots
const rom unsigned char address7[] = {	0x52,0x22,0x14,0x0C,0x1C,0x70};

// faders - 11 switches, 8 pots
const rom unsigned char address8[] = {	0x60,0x40,0x48,0x58,0x28,0x08,0x24,0x20,
                  						0x04,0x54,0x74,0x68,0x18,0x44,0x64,0x3B,
                  						0x45,0x1B,0x5B};
// easy access to the tables
const rom unsigned char *addr[8];
...
void Backplane_Comm(unsigned char LockedQframe) 
{
	...
	usb_in.SLOT[j].BYTES[0] = 0x00;				// Setup Data CMD_GROUP

	addrLength = region[reg].switches+region[reg].pots;
	addrLength = 42;

// RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL RECALL 
	if(totalRecall==2)
	{
DEBUG3=1;
	     USBDeviceTasks();
		for(l=0;l<addrLength;l++)		// change for region related loop times
		{
	        //MUX
			Address_MUX(12);
	        ...
	        {
	      	//      Read PORT
	            usb_in.SLOT[LockedQframe].BYTES[56+i]=GET_PORT;       // Dummy read
	            COMM_CLOCK = 1;   
	            for(j=0;j<4;j++){;}
	            COMM_CLOCK = 0;
	        }
//	       	USBDeviceTasks();
	        ...
	        {
		   		if(i==0)SET_PORT=*(addr[reg]+l);                 // set addresses from table
...
//	        USBDeviceTasks();												// Check USB pipe	
			...
			Address_MUX(bank);		// Change to bank
			MUX_ENABLE=0;													// Strobe the MUX address
	        COMM_READ;														// Set parallel port to READ
	        for(j=0;j<20;j++){;}												// waitloop to ensure the bank is responding
																// Read PORT
							

	        for(i=0;i<8;i++)												// 8 channels from each card
	        {																// Read PORT
				TempIn[l]=GET_PORT;
	            	COMM_CLOCK = 1;												// Clock
	            	for(j=0;j<2;j++){;}											// Wait loop
	            	COMM_CLOCK = 0;	
	            	for(j=0;j<6;j++){;}											// Wait loop
	        }
		...
		usb_in.SLOT[LockedQframe].BYTES[2]=reg+(bank<<4);	

		for(k=0;k<addrLength;k++)																			// Test for forbidden states to exclude empty ports
	    	{
			usb_in.SLOT[LockedQframe].BYTES[3+k]=TempIn[k];			// if everything ok, copy data
		}
...
DEBUG3=0;
	...
	if(dataState==1)
	{
	        ...
	        for(j=0;j<8;j++)												// if first command
	        {
	            usb_out.SLOT[LockedQframe].BYTES[(k*16)+(j*2)+9]=addCmd;		// set command on second byte
	        }
	        ...
	        for(i=0;i<4;i++)                                               // Write 2 x bytes to each card
	        {
	            SET_PORT=0x11;      		// Command for setup
	            for(j=0;j<2;j++){;}
	            COMM_CLOCK = 1;                                             // Clock
	            for(j=0;j<4;j++){;}
	            COMM_CLOCK = 0;
	...
	if(dataState==3)
	{
	    for(k=0;k<13;k++)													// Each USB packet has 48 bytes for 24 channels (3 banks).
	    {
	        ...
	        for(i=0;i<4;i++)												// 2 bytes x 8 channels from each card
	        {																// Read PORT
				TempIn[(k*3)+i]=GET_PORT;											// Store parallel data in TempIn
	            COMM_CLOCK = 1;												// Clock
	            for(j=0;j<2;j++){;}											// Wait loop
	            COMM_CLOCK = 0;	
	            for(j=0;j<6;j++){;}											// Wait loop
...
//	if(usb_out.SLOT[LockedQframe].CMD.CMD_GROUP==0 && !dataState)
	if(dataState<2 && totalRecall<2)
	{
		if(dataState)addCmd=0x11;
		if(totalRecall)addCmd=0x12;
...
// eq 2 of 2 - 5 pots (last one is type)
  ...
  region[4].pots 	= 5;
// sends 1 of 2 - 15 switches
  region[5].switches 	= 15;
  region[5].pots 	= 0;
// sends 2 of 2 - 6 pots
  region[6].switches 	= 0;
  region[6].pots 	= 6;
// faders - 11 switches, 8 pots
  region[7].switches 	= 11;
  region[7].pots 	= 8;
...
DEBUG2=1;
	...
	//		usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x00;
			...
			if(totalRecall==3)
			{
				usb_in.SLOT[LockedQframe].CMD.CMD_GROUP = 0x02;
...
//		else dataState=0;
		...
		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x04)
		{
DEBUG2=1;
			dataState=1;  
DEBUG2=0;
...
DEBUG2=0;
 		...
 		if((usb_out.SLOT[slot].BYTES[0]&0xFC) == 0x18)
		{
DEBUG2=1;
			totalRecall=0;  
			setMPX(0);
