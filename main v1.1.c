		...
		{	
			...
			switch (MidiIn)
			{
				case 0x7F:													// 	0x7F	-> SysEx FullFrame 2nd byte 
					if(MidiState==0)										// Initial 0x7F
					{
						MidiState=1;
					}
					if(MidiState==1)										// Second 0x7F
					{
						MidiState=2;
					}
					if(MidiState==2)										// Last 0x7F
					{
						MidiState=0;
					}
					break;
...
/*
		...
		SET_PORT=LockedQframe;											// I2C card must get channel no		       
        ...
        {
      		//      Read PORT
       //     usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[i+((LockedQframe&1)<<2)]=GET_PORT;       // Place byte in the appropriate slot
			TempIn[48+i]=GET_PORT;
            COMM_CLOCK = 1;      
            //Delay
            for(j=0;j<4;j++)
            {
        ...
        {
            SET_PORT=usb_out.SLOT[(LockedQframe>>1)+2].MF_DATA[i+((LockedQframe&1)<<2)];      // Take byte from the right slot 
        //    SET_PORT=0x08;
            //Delay
            for(j=0;j<2;j++)
            {
...
// SSL CARDS SSL CARD SSL CARDS SSL CARD SSL CARDS SSL CARD SSL CARDS SSL CARD SSL CARDS SSL CARD SSL CARDS SSL CARD SSL CARDS SSL CARD
   ...
   //     COMM_DIR=1;                     // This is not used (yet)
        ...
        {
      //      //Read PORT
      //       usb_in.SLOT[LockedQframe].SSL_DATA[(k*16)+i]=GET_PORT;       // Place byte in the appropriate slot
			TempIn[(k*16)+i]=GET_PORT;
            COMM_CLOCK = 1;      
            //Delay
            for(j=0;j<4;j++)
            {
  ...
  //      COMM_DIR=0;                                                     // Not used
    ...
    for(k=0;k<3;k++)                    // Test for forbidden states to exclude empty ports
    {
        for(i=0;i<8;i++)                                             
		{
			if((TempIn[((k*16)+(i*2)+1)]&0x0C)==0x0C ||
			(TempIn[((k*16)+(i*2)+1)]&0xC0)==0xC0 ||
			DataSafe==0)
			{
				usb_in.SLOT[LockedQframe].SSL_DATA[(k*16)+(i*2)+1]&=0x03;
			}
			else
			{
				usb_in.SLOT[LockedQframe].SSL_DATA[(k*16)+(i*2)]=TempIn[(k*16)+(i*2)];
    ...
    for(i=0;i<2;i++)                                               
	{
		...
		if((TempIn[49+(i*2)]&0x30)!=0x30 &&
		(TempIn[49+(i*2)]&0xC0)!=0xC0 &&
		DataSafe)				// Both touch press and release are set
		{
			usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)+((LockedQframe&1)<<2)]=TempIn[48+(i*2)];
			usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)+1+((LockedQframe&1)<<2)]=TempIn[49+(i*2)];
		}
		else
		{
			usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)+((LockedQframe&1)<<2)]=0;
			usb_in.SLOT[(LockedQframe>>1)+2].MF_DATA[(i*2)+1+((LockedQframe&1)<<2)]=0;
		}
...
{   
	...
	if(LockedQframe==0)DEBUG3=1;
                ...
                usb_in.SLOT[LockedQframe].MTC_PROP.FPS = FPS;    
				for(i=0;i<64;i++)
					{
