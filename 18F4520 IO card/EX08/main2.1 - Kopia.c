...
#define MUTE_IN	  				PORTAbits.RA3		// originally RA1. First card changed to RC3. Why didn't RA1 work?
...
//		OUTdata.data[1]	= 0x13;
		...
		if (INTCONbits.INT0IF)									// Interrupt flag for RB0
		{
			ClrWdt();											// Is this bad?
			cnt=0;	
			SSP_write();										// Fisrt send the 16 bytes
...
void Init(void)
{
	...
	TRISA = 0x00;					// Port A: RA0=ADC, RA1=LED, RA2=VREF-, RA3=VREF+, RA6=OSC;
...
//	INdata.data[1]=0x04;
	...
	// Shift the data left. Msb first.	
			...
			if(SWITCH_DATA) OUTdata.data[((7-i)*2)+byteSwitch]+=0x01;
