...
// v3.2
			if(1)
			{
				if (!ParallelPortflag)								// Interrupt flag for RB0
				{
					...
					if(fpsCntr>16) fpsTmr=SLOW;
			//		if(fpsCntr<15) fpsTmr=FAST;
					cueTableCntr=0;
					fpsCntr=0;
				}
			}
	...
	{
		VCAfader[muxIndex] = ADRESH;	
		VCAfader[muxIndex] = (VCAfader[muxIndex]<<8)+ADRESL;				// put the word together

		if(((LocalMuteBits>>muxIndex)&1) != MUTE_IN)						// Check for MUTE change
		{
			if(MUTE_IN) 
			{
				OUTdata.channel[muxIndex].mute_press = 1;				// Event flag for mute press
				OUTdata.channel[muxIndex].mute_release = 0;					// clear event flag
				bit_set(LocalMuteBits,muxIndex);							// Set bit in a storage byte

			}
			else
			{
				OUTdata.channel[muxIndex].mute_press = 0;				// clear event flag
				OUTdata.channel[muxIndex].mute_release = 1;				// Event flag for mute release
				bit_clr(LocalMuteBits,muxIndex);							// Clear bit in a storage byte
			}
		}
		else
		{
	...
	while(1)
	{
		...
		if(cueTableFlag)
		{
			...
			else				// odd
			{
				...
				VCAout[muxIndex]=VCAoutOld[muxIndex];
				DACdelta[muxIndex]=DACsetPoint[muxIndex];
				DACdelta[muxIndex]=(DACdelta[muxIndex]-VCAoutOld[muxIndex])/16;
				VCAoutOld[muxIndex]=DACsetPoint[muxIndex];
...
void Set_Leds(void)
	...
	{
		LED_DATA=0;
		if(INdata.channel[7-i].status==1 || INdata.channel[7-i].status==3)LED_DATA=1;
		if(INdata.channel[7-i].status==2)LED_DATA=BlinkLED;	
		CLOCK_PIN=1;
		CLOCK_PIN=0;
	}
