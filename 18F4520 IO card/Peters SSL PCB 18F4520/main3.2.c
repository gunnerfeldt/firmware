...
// v3.2
			...
			{	
				cnt=0;
				LATD = VCAgroupValue[0];							// start with first byte on parallel port
				while(cnt<16)
				{
					while(SSP_clk==0){;}						// Wait for fisrt clock
					cnt++;										// count index
					LATD = VCAgroupValue[cnt];					// Put byte on parallel port
					while(SSP_clk==1){;}						// Data must remain on the port until clock falls
					if(!SSP_en){break;}							// if CS pin releases, abort
				}												// loop 4 times
				SSP_read();
...
/*
			...
			// Group recall - This can be done in a single transfer. When the command comes.
// v3.2
			if(grpRecall)
			{		
				cnt=0;										// loop 4 times
				...
				{
					while(SSP_clk==0){;}						// Wait until clock rises
					VCAgroupValue[cnt]=PORTD;					// read and store port in a buffer
					cnt++;										// count index
					while(SSP_clk==1){;}						// hold until clock falls
					if(!SSP_en){break;}						// if CS pin releases, abort
				}	
...
// v3.2
						...
						if((INbuffer.cmd[n].cmd == 2) && !MOTORFADERS)
						{
							totalRecallState=1;
// v3.2
							automationState=0;
						...
						if(INbuffer.cmd[n].cmd == 3)			// group snapshot
						{
							grpSnapshot=1;
// v3.2
							automationState=0;
						}
						if(INbuffer.cmd[n].cmd == 4)			// group recall
						{
				//			VCAgroupValue[n*2]=INbuffer.data[n*2];
				//			VCAgroupValue[(n*2)+1]=(INbuffer.data[(n*2)+1]&0x60)>>5;
...
if(totalRecallState || hardwareReportState || grpSnapshot || grpRecall) return;
...
*/
	...
	if(VCAgroupFlag)		// flag is set for toggling fader mode
	{
		if(!byteSwitch)
		{
			VCAgroup=!VCAgroup;	// only on the first byte, toggle mode
		}
	...
	for(n=0;n<8;n++)
	{		
// Test to have data ready on the SDO pins first !!!!
/*  
	...
	// Next 8 bits keep clock inactive
...
*/
// Now do it again with clock High
		...
		if(byteSwitch)								// only after second byte has been read
		{
			...
			else
			{
				...
				for(i=0;i<8;i++)						// 
				{
					...
					VCAgroupValue[(i*2)+1]=VCAGroupValueTemp;				// Store Group Level for snapshots
					VCAGroupValueTemp<<=8;
					VCAGroupValueTemp+=tempOUTdata[(i*2)];
					VCAgroupValue[i*2]=tempOUTdata[(i*2)];				// Store Group Level for snapshots
