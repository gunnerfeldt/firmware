	...
	if (INTCONbits.TMR0IF)										// Timer base for creating slots in between transfers
		{
			...
			MFtimer++;
			if(MFtimer==5)MFtimerTick=1;						// Motorfader serial data. Second byte
			ledTRcntr--;
			if(!ledTRcntr)
			{
...
//	RCONbits.IPEN = 1;		// Enables Priorites
	...
	T0_8BIT &
	T0_SOURCE_INT &
	T0_PS_1_32 );		// Frequency for writing to ADC/DAC
...
//	HANDSHAKE=MOTORFADERS=1;
	while(1)
	{
		...
		if(ParallelPortflag)							// New Data arrived
		{
			...
			{
				if((INbuffer.cmd[n].flag==0))				// same as id change flag = 0x10
				{
				...
				else
				{
					...
					{
						if(INbuffer.cmd[n].cmd == 0)			// HW Report
						{
							INdata.word[n]=INbuffer.word[n];		// Copy buffer
						}
...
// v3.2
						...
						if(INbuffer.cmd[n].cmd == 3 && MOTORFADERS)			// group snapshot
						{
							grpSnapshot=1;
// v3.2
							automationState=0;
...
/*
						if(INbuffer.cmd[n].cmd == 4 && MOTORFADERS)			// group recall
						{
							grpRecall=1;
						//	VCAgroupLin.bytes[(n*2)]=INbuffer.vcaGroup[n].lo_byte;
						//	VCAgroupLin.bytes[(n*2)+1]=INbuffer.vcaGroup[n].hi_byte;
						}
*/
					}
				}
...
if(totalRecallState || hardwareReportState) return;
	...
	if(VCAgroupState==1 && !byteSwitch)		// First state of VCA group switch detected
	{
		...
		if(byteSwitch)		// First state of VCA group switch detected
		{
		//	VCAgroupState=2;		// Next state
		}
	}
	...
	if(grpRecall && !byteSwitch)		// flag is set for recalling VCA group level
	{
		for(i=0;i<8;i++)
		{
		//	INdata.vcaGroup[i].lo_byte=VCAout[i]&0xff;
		//	INdata.vcaGroup[i].hi_byte=(VCAout[i]>>8);
			INdata.data[i*2]=VCAgroupLin.bytes[i*2];	
			INdata.data[(i*2)+1]=0x13+(VCAgroupLin.bytes[(i*2)+1]<<5);	
		}
	//	grpRecall=0;
