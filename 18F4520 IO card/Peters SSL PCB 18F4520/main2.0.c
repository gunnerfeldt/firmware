...
#define MUTE_IN	  				PORTCbits.RA3		// originally RA1. First card changed to RC3. Why didn't RA1 work?
...
//			Read_Switches();
		...
		if(Timer_Flag)						// New Data arrived
		{
/*
			for(n=0;n<8;n++)						// loop thru channels
			{	
...
*/
		...
		{
			if((Q_Frame & 0x3)==0x0) Motor_Faders(0);		// Q frame 0
			if((Q_Frame & 0x3)==0x1) Motor_Faders(1);		// Q frame 1
			Q_Frame_Flag=0;
			Q_Frame++;
		}
...
//	INdata.data[1]=0x04;
		...
		{		
			...
			for(i=0;i<8;i++)	// SDO (TRIM LED)						
				{
				OutByte=(INdata.data[((7-i)*2)+byteSwitch]>>(7-n));				// Copy shifted Out Data
		 		MF_REG_DATA=~(OutByte & 1);								// Set Data Out pin
//		 		MF_REG_DATA=0;								// Set Data Out pin
			...
			for(i=0;i<8;i++) 	// SCK (ABS LED)	
				{
				MF_REG_DATA = 0;							// This turns rising to SPI clock
				CLOCK_PIN=1;
				CLOCK_PIN=0;	
				}
			LED_LOAD=1;										// Strobe LED chip
			LED_LOAD=0;
												// Read Data In - Maintain Data Out
			for(i=0;i<8;i++)	// SDO (TRIM LED)						
				{
				OutByte=(INdata.data[((7-i)*2)+byteSwitch]>>(7-n));				// Copy shifted Out Data
		 		MF_REG_DATA=~(OutByte & 1);								// Set Data Out pin
//		 		MF_REG_DATA=0;								// Set Data Out pin
			...
			for(i=0;i<8;i++) 	// SCK (ABS LED)	
				{
				MF_REG_DATA = 1;							// This turns to falling SPI clock
				CLOCK_PIN=1;
				CLOCK_PIN=0;	
				}
			LED_LOAD=1;										// Strobe LED chip
			...
			for(i=0;i<8;i++) 	// Read SDI (Switch Reg)	
			{
				if(!SWITCH_DATA)
				{
					OUTdata.data[((7-i)*2)+byteSwitch]+=1;				// Data In pins
				}
				OUTdata.data[((7-i)*2)+byteSwitch] <<= 1;					// Shift In Data to the Left
				CLOCK_PIN=1;
				CLOCK_PIN=0;	
			}
...
void Read_Switches(void)
	...
	if(SwitchBits!=StoredSwitchBits) {							// If previous 8 bits are NOT equal with the current 8 bits:
		...
		for (n=0;n<8;n++){
			...
			OUTdata.channel[7-n].card_id=0x01;							// If we're reading  switches card ID is 0x01;
		}
	}
