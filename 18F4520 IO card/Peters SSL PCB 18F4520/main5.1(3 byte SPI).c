...
void Motor_Faders(unsigned char byteSwitch)
{
	...
	// Msb out first. "byteSwitch" determines if it's byte 0 or 1
			...
			if(byteSwitch==0)
			{
				OutByte=((INdata.data[((7-i)*2)]>>(7-n))&0b00111111);
				OutByte+=0b01000000;
			}
			if(byteSwitch==1)
			{
				OutByte=(((INdata.data[((7-i)*2)]>>(7-n))&0b11000000)>>6);
				OutByte+=(((INdata.data[((7-i)*2)+1]>>(7-n))&0b00001111)<<4);
				OutByte+=0b10000000;
			}
			if(byteSwitch==2)
			{
				OutByte=((INdata.data[((7-i)*2)+1]>>(7-n))&0b11110000)>>4;
				OutByte+=0b11000000;
			}
