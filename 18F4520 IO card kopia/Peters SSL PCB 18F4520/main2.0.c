...
void Motor_Faders(void)
    ...
    for(n=0;n<8;n++)
    {
        ...
        {
            LED_MUTE_DATA=0;				// Dummy data to pass thru MUTE reg
            CLOCK_PIN=1;
            CLOCK_PIN=0;
        }
                                                        // Data
        ...
        {
            OutByte=INdata.data[i*2]>>n;		// Copy shifted Out Data
            LED_MUTE_DATA=OutByte & 1;                  // Set Data Out pin
            CLOCK_PIN=1;
            CLOCK_PIN=0;
        }
                                                        // Clock
        ...
        {
            LED_MUTE_DATA = 1;                          // This turns rising to SPI clock
            CLOCK_PIN=1;
            CLOCK_PIN=0;
        }
        ...
        for(i=0;i<8;i++)                                // SCK (ABS LED)
        {
            LED_MUTE_DATA = 0;                          // This turns to falling SPI clock
            CLOCK_PIN=1;
            CLOCK_PIN=0;
        }
        LED_LOAD=1;					// Strobe LED chip
        ...
        SWITCH_LOAD = 0;				// Strobe SDI(Switch Reg)
        SWITCH_LOAD = 1;

        for (m=0;m<8;m++)
        {
            OUTdata.data[m*2]+=SWITCH_DATA;		// Data In pins
            OUTdata.data[m*2] <<= 1;			// Shift In Data to the Left
            CLOCK_PIN = 1;
            CLOCK_PIN = 0;
        }

        OUTdata.data[n*2]+=SWITCH_DATA;			// Data In pins
        OUTdata.data[n*2] <<= 1;			// Shift In Data to the Left
    }

    for(i=0;i<8;i++)					// Last, send Mute data
    {
