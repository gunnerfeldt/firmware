...
unsigned int const ONLINETIME = 200;
...
// ************************ P W M  I n t e r r u p t ******************************************
	...
	{		
		postScaler++;
		SPItimer++;
		TicksPerFrame++;														// Count how many ticks

		// 20ms after last 2nd byte came in
		if(TicksPerFrame==75)		// Must Trigger once (65)
		{
			SSPCON1bits.SSPEN=1;	// Start SPI
		}

		if(TicksPerFrame>130)		// If lost reset after 1 frame;
		{
			TicksPerFrame=0;
		}
...
//				if(LocalTouch)SettleCnt=0;
				...
				if(NewDataIn && FaderReady )
				{
					...
					//	FaderGoal=(PreviousFaderGoal+((inbuffer.word&0x3ff)*3))/4;
...
//			if(faderMode==1) Update_LED_Display((FaderPosition&0x01FF) >> 3);
...
/*
	SMPMID 	= Input sampled in the middle of data out ?
	SMPEND	= Input sampled in the end of data out ?
	MODE 	= 0 - Active high, Rising edge
			  1 - Active high, Falling edge
			  2 - Active low, Rising edge
			  3 - Active low, Falling edge
