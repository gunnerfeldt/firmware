...
unsigned int const ONLINETIME = 400;		// ms
unsigned int const ERRORTOL = 10;			// steps
...
unsigned int initSetPoint=0;
// *************************** F  A  D  E  R    I  D  ************************************
// *************************** F  A  D  E  R    I  D  ************************************
// *************************** F  A  D  E  R    I  D  ************************************
// *************************** F  A  D  E  R    I  D  ************************************
unsigned char FADER_ID = 0;
unsigned char xID=0;
unsigned char SSLchannel=0;
...
// ************************ P W M  I n t e r r u p t ******************************************
	if (INTCONbits.TMR0IF)									// Timer base
	{
		...
		{ 
			FaderPosition=((FaderPosition*1)+(Read_ADC()*3))/4; 
...
void main (void)
{
	...
	static signed int lastSetPoint;
	static signed int fabsError;
	Init();
...
//			if(fabs(setPoint-FaderPosition)>ERRORTOL)onlineCntr=ONLINETIME;
			fabsError=setPoint;
			fabsError=fabsError-FaderPosition;
			fabsError=fabs(fabsError);
			if(fabsError>ERRORTOL)onlineCntr=ONLINETIME;
			if(setPoint!=lastSetPoint)onlineCntr=ONLINETIME;
...
//			if(InitCnt>(delayFactor-(FADER_ID+1)))
			if(InitCnt>1)
			{
				TestPhase=1;
				InitCnt=0;
