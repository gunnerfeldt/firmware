...
{	
	...
	static unsigned char postScaleCntr=0;
...
//				if(LocalTouch)SettleCnt=0;
				...
				if(NewDataIn)
				{
					if(faderMode==1)					// Standard Control Surface Mode
					{
						...
						if(FaderGoal>1022)FaderGoal=1022;
					}
...
//			if(faderMode==1) Update_LED_Display(SettleCnt/10);
